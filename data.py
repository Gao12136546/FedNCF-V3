...
class SampleGenerator(object):
    ...
    def __init__(self, config, ratings):
        ...
        self.pseudo_item_embeddings = None
    ...
    def store_pseudo_train_data(self, num_negatives):
        """store all the train data as a list including users, items and ratings. each list consists of all users'
        information, where each sub-list stores a user's positives and negatives"""
        users, items, ratings = [], [], []
        pseudo_train_data = {}
        for userId,emb in self.pseudo_item_embeddings.items():
            user_neg_items = []
            for i in range(emb.shape[0]):
                user_neg_items.append(random.sample(self.negatives[self.negatives['userId'] == userId]['negative_items'].tolist()[0],num_negatives))
            pseudo_train_data[userId] = user_neg_items
